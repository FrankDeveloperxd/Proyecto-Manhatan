// src/features/sensors/SensorDetail.tsx
import { useEffect, useMemo, useState, useRef } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import type { Sensor } from "./types";
import { subscribeSensor } from "./api";
import { client as mqtt, DEFAULT_TOPIC } from "../../lib/mqttClient";
import MapLive from "./MapLive";

const nowTs = () => ({ seconds: Math.floor(Date.now() / 1000) });

// historial en memoria
type HistoryItem = {
  id: number;
  ts: number;
  topic: string;
  raw: string;
  lat?: number;
  lng?: number;
};

const MAX_HISTORY = 50;

// Tecsup Arequipa (fallback inicial)
const TEC_SUP = { lat: -16.409047, lng: -71.537451 };

export default function SensorDetail({
  sensor,
  onClose,
}: {
  sensor: Sensor;
  onClose: () => void;
}) {
  const [live, setLive] = useState<Sensor | undefined>(sensor);
  const [history, setHistory] = useState<HistoryItem[]>([]);

  const tdRef = useRef(new TextDecoder());
  const navigate = useNavigate();
  const location = useLocation();
  const mounted = useRef(true);

  useEffect(() => {
    mounted.current = true;
    return () => {
      mounted.current = false;
    };
  }, []);

  // Mantener espejo con Firestore (si usas docs)
  useEffect(() => {
    if (!sensor?.id) return;
    const off = subscribeSensor(sensor.id, (s) => {
      if (!mounted.current) return;
      setLive(s);
    });
    return () => off && off();
  }, [sensor?.id]);

  // Suscripción directa a MQTT (Flespi) -> topic FINAL: esp32/test
  useEffect(() => {
    const base = DEFAULT_TOPIC || "esp32/test"; // viene de .env ó constante
    const topics = [base];

    console.log("[MQTT] Suscribiendo a", topics);

    mqtt.subscribe(topics, (err, granted) => {
      console.log("[MQTT] subscribed", { base, err, granted });
    });

    const onMsg = (topic: string, payload: any) => {
      if (topic !== base) return; // solo nos importa esp32/test

      const text =
        typeof payload === "string" ? payload : tdRef.current.decode(payload);
      console.log("[MQTT] msg", topic, text);

      const tsMs = Date.now();

      // Intentar parsear JSON
      let json: any = null;
      try {
        json = JSON.parse(text);
      } catch (e) {
        console.warn("[MQTT] payload no-JSON", e);
      }

      // Extraer coordenadas si existen
      let lat: number | undefined;
      let lng: number | undefined;

      if (json && typeof json === "object") {
        const rawLat =
          json.lat ??
          json.latitude ??
          json.Latitude ??
          json.LAT ??
          json.LonLat?.lat;
        const rawLng =
          json.lng ??
          json.lon ??
          json.longitude ??
          json.Longitude ??
          json.LonLat?.lng;

        if (rawLat !== undefined && rawLng !== undefined) {
          const nLat = Number(rawLat);
          const nLng = Number(rawLng);
          if (!Number.isNaN(nLat) && !Number.isNaN(nLng)) {
            lat = nLat;
            lng = nLng;
          }
        }
      }

      // Actualizar estado "live" del sensor
      setLive((prev) => {
        if (!prev) return prev;

        const next: Sensor = {
          ...prev,
          lastSeenAt: nowTs(),
          lastReading: json ?? text, // si no es JSON, guardamos string
        };

        if (lat !== undefined && lng !== undefined) {
          next.location = { lat, lng };
        }

        return next;
      });

      // Agregar al historial en memoria
      setHistory((prev) => {
        const item: HistoryItem = {
          id: tsMs,
          ts: tsMs,
          topic,
          raw: json ? JSON.stringify(json) : text,
          lat,
          lng,
        };
        const merged = [item, ...prev];
        if (merged.length > MAX_HISTORY) {
          return merged.slice(0, MAX_HISTORY);
        }
        return merged;
      });
    };

    mqtt.on("message", onMsg);

    return () => {
      mqtt.off("message", onMsg);
      mqtt.unsubscribe(topics);
    };
  }, []);

  const status = useMemo(() => computeStatus(live), [live]);
  const coord = live?.location;

  const mapPos = coord ?? TEC_SUP; // Tecsup si aún no hay fix

  return (
    <>
      <div className="fixed inset-0 z-40 flex items-center justify-center">
        <div className="absolute inset-0 bg-black/40" onClick={onClose} />
        <div className="bg-white rounded-2xl p-0 w-[min(1100px,95%)] max-h-[90vh] overflow-auto shadow-xl">
          {/* Header */}
          <div className="px-6 py-4 bg-gradient-to-r from-violet-600 via-indigo-600 to-cyan-600 text-white">
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold">
                Sensor • {live?.workerName || sensor.workerName}
              </div>
              <button
                className="text-white/90 hover:text-white"
                onClick={onClose}
              >
                Cerrar
              </button>
            </div>
            <div className="text-sm text-white/85">
              Topic: <strong>{DEFAULT_TOPIC || "esp32/test"}</strong>
            </div>
          </div>

          <div className="p-6 grid lg:grid-cols-2 gap-5">
            {/* Estado + última lectura */}
            <section className="rounded-2xl border bg-white/80 p-4">
              <div className="flex items-center justify-between">
                <h4 className="font-medium">Estado</h4>
                <StatusBadge status={status} />
              </div>
              <div className="mt-3 text-sm text-slate-600">
                Último visto:{" "}
                <span className="font-medium">
                  {live?.lastSeenAt?.seconds
                    ? new Date(
                        live.lastSeenAt.seconds * 1000
                      ).toLocaleString()
                    : "—"}
                </span>
              </div>
              <div className="mt-3">
                <div className="text-sm text-slate-600 mb-1">
                  Última lectura
                </div>
                <pre className="p-3 rounded-xl bg-slate-50 text-sm overflow-auto">
                  {JSON.stringify(live?.lastReading ?? { msg: "—" }, null, 2)}
                </pre>
              </div>
            </section>

            {/* Mapa + info de ubicación + historial de coordenadas */}
            <section className="rounded-2xl border bg-white/80 p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <h4 className="font-medium">Ubicación en tiempo real</h4>
                <a
                  className="text-sm text-sky-700 hover:underline"
                  href={`https://maps.google.com/?q=${mapPos.lat},${mapPos.lng}`}
                  target="_blank"
                  rel="noreferrer"
                >
                  Abrir en Maps
                </a>
              </div>

              {/* Texto de coordenadas actuales */}
              <div className="text-xs text-slate-600">
                Coordenadas actuales:{" "}
                {coord ? (
                  <span className="font-mono">
                    {coord.lat.toFixed(6)}, {coord.lng.toFixed(6)}
                  </span>
                ) : (
                  <span className="italic text-slate-500">
                    Sin coordenadas. Mostrando Tecsup Arequipa como referencia.
                  </span>
                )}
              </div>

              {/* Mapa */}
              <div className="rounded-xl overflow-hidden border">
                <MapLive
                  lat={mapPos.lat}
                  lng={mapPos.lng}
                  showMarker={!!coord}
                  height={260}
                />
              </div>

              {/* Historial de mensajes */}
              <div className="mt-1">
                <div className="text-xs font-medium text-slate-700 mb-1">
                  Historial de mensajes (últimos {MAX_HISTORY})
                </div>
                <div className="rounded-xl border border-slate-200 bg-slate-50 max-h-40 overflow-y-auto text-xs">
                  {history.length === 0 ? (
                    <div className="px-3 py-2 text-slate-500">
                      Aún no hay datos del dispositivo.
                    </div>
                  ) : (
                    <ul className="divide-y divide-slate-200">
                      {history.map((h, idx) => (
                        <li key={h.id} className="px-3 py-2">
                          <div className="flex justify-between items-center">
                            <span className="text-[11px] text-slate-500">
                              #{history.length - idx} •{" "}
                              {new Date(h.ts).toLocaleTimeString()}
                            </span>
                            {h.lat !== undefined && h.lng !== undefined && (
                              <span className="font-mono">
                                {h.lat.toFixed(5)}, {h.lng.toFixed(5)}
                              </span>
                            )}
                          </div>
                          <div className="mt-1 font-mono text-[11px] text-slate-700 break-all">
                            {h.raw}
                          </div>
                        </li>
                      ))}
                    </ul>
                  )}
                </div>
                <p className="text-[11px] text-slate-400 mt-1">
                  * Mapa real (OpenStreetMap). El punto rojo aparecerá cuando el
                  mensaje incluya lat / lng.
                </p>
              </div>
            </section>
          </div>
        </div>
      </div>

      {/* Overlay de EMERGENCIA a pantalla completa (lo mantengo igual) */}
      {live?.emergency ? (
        <div className="fixed inset-0 z-50 grid place-content-center bg-red-900/95 text-white p-6">
          <div className="max-w-3xl text-center rounded-lg bg-white/10 p-6">
            <div className="text-4xl font-extrabold mb-3">¡EMERGENCIA!</div>
            <div className="text-xl mb-2">
              {live.workerName} • {live.workerId}
            </div>
            {live.location ? (
              <div className="mb-4">
                Ubicación: {live.location.lat.toFixed(6)},{" "}
                {live.location.lng.toFixed(6)}
              </div>
            ) : (
              <div className="mb-4">Sin ubicación válida (esperando fix)</div>
            )}

            <div className="flex gap-3 justify-center">
              {live.location && (
                <a
                  className="rounded-lg px-4 py-2 bg-white text-red-800 font-semibold"
                  target="_blank"
                  rel="noreferrer"
                  href={`https://maps.google.com/?q=${live.location.lat},${live.location.lng}`}
                >
                  Abrir en Maps
                </a>
              )}
              <button
                className="rounded-lg px-4 py-2 bg-white/20"
                onClick={() => alert("Notificar contactos (implementar)")}
              >
                Notificar contactos
              </button>
              <button
                className="rounded-lg px-4 py-2 bg-white/30"
                onClick={() => {
                  setLive((prev) =>
                    prev ? { ...prev, emergency: false } : prev
                  );
                  onClose();
                }}
              >
                Cerrar
              </button>
            </div>
          </div>
        </div>
      ) : null}
    </>
  );
}

/* ---------- lógica de estados ---------- */
type StatusKey =
  | "no-data"
  | "connected"
  | "unstable"
  | "disconnected"
  | "gps-lost"
  | "emergency";

function computeStatus(s?: Sensor): StatusKey {
  if (!s) return "no-data";
  if (s.emergency) return "emergency";
  if (!s.lastSeenAt?.seconds) return "no-data";
  const age = Date.now() - s.lastSeenAt.seconds * 1000;
  if (!s.location) return age < 5 * 60_000 ? "gps-lost" : "disconnected";
  if (age < 60_000) return "connected";
  if (age < 5 * 60_000) return "unstable";
  return "disconnected";
}

function StatusBadge({ status }: { status: StatusKey }) {
  const map = {
    "no-data": {
      text: "Sin datos",
      cls: "bg-slate-100 text-slate-700 border",
    },
    connected: {
      text: "Conectado",
      cls: "bg-emerald-100 text-emerald-700 border border-emerald-200",
    },
    unstable: {
      text: "Intermitente",
      cls: "bg-amber-100 text-amber-800 border border-amber-200",
    },
    disconnected: {
      text: "Desconectado",
      cls: "bg-slate-200 text-slate-700 border",
    },
    "gps-lost": {
      text: "Sin señal GPS",
      cls: "bg-sky-100 text-sky-800 border border-sky-200",
    },
    emergency: {
      text: "¡Emergencia!",
      cls: "bg-red-100 text-red-700 border border-red-200",
    },
  } as const;
  const m = map[status];
  return (
    <span className={`px-2 py-1 rounded-full text-xs ${m.cls}`}>
      {m.text}
    </span>
  );
}
