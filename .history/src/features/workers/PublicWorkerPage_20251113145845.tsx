...
import MapLive from "../sensors/MapLive";
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <Card title="Ubicación y Cómo llegar" icon="profile" delay="210ms">
            {/*
              ...
              Puedes cambiar estas coordenadas por las del lugar real del trabajador.
              Ahora está Tecsup Arequipa 👇
            */}
            {(() => {
              const dstLat = -16.409047;   // Tecsup approx
              const dstLng = -71.537451;

              return (
                <div className="space-y-3">
                  {/* Mapa reutilizando el componente que ya tienes (Leaflet, sin API key) */}
                  <div className="w-full h-64 rounded-xl overflow-hidden border">
                    <MapLive lat={dstLat} lng={dstLng} showMarker={true} height={256} />
                  </div>

                  <div className="text-slate-600 text-sm">
                    Punto de referencia del trabajador (Tecsup Arequipa).
                  </div>
                  ...
                  <button
                      ...
                      if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                          (pos) => {
                            ...
                            // Abre Google Maps con ruta a pie (no necesita API key)
                            window.open(
                              `https://www.google.com/maps/dir/?api=1&origin=${myLat},${myLng}&destination=${dstLat},${dstLng}&travelmode=walking`,
                              "_blank"
                            );
                          },
                          () => {
                            // Si falla el GPS, abre solo el destino
                            window.open(
                              `https://www.google.com/maps/dir/?api=1&destination=${dstLat},${dstLng}&travelmode=walking`,
                              "_blank"
                            );
                          }
                        );
                      } else {
                        // Sin geolocalización, solo destino
                        window.open(
                          `https://www.google.com/maps/dir/?api=1&destination=${dstLat},${dstLng}&travelmode=walking`,
                          "_blank"
                        );
                    ...
                    className="w-full rounded-xl bg-indigo-600 text-white py-3 text-center font-semibold shadow hover:bg-indigo-700 transition"
                  >
